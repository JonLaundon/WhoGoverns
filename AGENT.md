...
## Build-out method — read `docs/2026-07-21-buildout-strategy-v0-1.md`
THE ONE RULE: **domain EXTRACTION is systematic** (driven by the seed instrument's structure + breadcrumbs, to breadcrumb-exhaustion — never scoped to what a target case needs); **CHAIN assembly** (a delivery chain / fusion / retrodiction) is goal-directed but runs in `/analysis` AGAINST a completed domain, as the audit, never the driver. Test: if you can only name the next provision from "what my case needs", not from the domain's structure or a breadcrumb, you've slipped — return to the bloom. Method: theme-seeded blooms (a policy domain is the seed), catalogued against the Over-Ruled 115-Act list (discovery only, #24), run until the domain's distinct-instrument count goes flat. The shared general-law spine (Enterprise Act 2002, Competition Act 1998, Companies/Insolvency Acts, FSMA 2000) is extracted ONCE and inherited by every domain. The complete-body role/responsibility card is built only when a body's breadcrumb is exhausted (gated; recomputes on new powers).
