...
## Powers layer (Spiral 2, Annex A6 — ACTIVE)
Six new types, `data/<type>.json` via store like the rest: **powers, duties, vetoes** (the operative records) + **instruments, provisions, definitions** (the legislative graph — legislation as a versioned codebase). Vocab in `vocab/`: power_type, duty_type, veto_type, veto_strength, modality, legal_effect, instrument_type, condition_type, legislative_edge_type.
- **Holder is polymorphic:** `holder_type` (`office`|`body`), `office_id` when an office holds it (a corporation sole — "the Secretary of State may…"), `body_id` always set (the holder or hosting department).
- **modality** is const per type (`power`|`duty`|`veto`) — it self-describes and cannot disagree with the record type. Getting may (power) vs must (duty) vs block (veto) right is the extractor's single most important job.
- **power_basis** (required, decision #21): Spiral 2 sets `statutory` on every Power — full stop. `prerogative`/`common_law`/`contractual` exist so a statutorily-empty domain reads as "prerogative likely", not "no power"; they are NOT extracted yet (no provision to cite). Conventions (Sewel) are never recorded (#22). Retained-EU law and the ECHR both enter as `statutory` (SIs; the Human Rights Act 1998) — never a separate basis.
- **Veto = a DERIVED projection, never a primitive.** A veto is emitted ONLY as the blocking projection of a consent/approval/concurrence/call-in/objection/licence-refusal/designation **Power**: `derived_from_record_id` (the canonical `power-*`) and `decision_affected` are both REQUIRED. The canonical record is the Power; the Veto adds the *target* (whose decision is blocked).
- **STRICT veto test:** only a genuine statutory block. "must be consulted" → a consultation **Duty**, not a veto. Soft influence / "have regard to" / relationship leverage → **not recorded at all**. There is NO "Responsibility" record type — responsibilities decompose into powers+duties or live in the body summary, never as uncited data.
- **One provision, one canonical record (A2.5):** all records from a provision share `provision_key`; the canonical one has `derived_from_record_id` null; validate rejects two canonical records sharing a `provision_key`.
- **Provision text is NOT stored:** the Provision node carries the legislation.gov.uk point-in-time `url` + `version_date` + a `content_hash` (drift detection). `outstanding_effects=true` flags unapplied amendments (consolidation lag). AMENDS history is deferred (consolidated "as amended" text only).
- **Assurance tiers** (verification_status): `unverified` → `machine_verified` (passed the schema-blind eval, no human) → `single_checked` / `double_checked` (human, A10). Machine-verified may publish WITH an honest label; high-impact vetoes still need a human double-check.
